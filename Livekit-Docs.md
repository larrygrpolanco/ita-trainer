...
The simplest way to get your first agent running is with one of the following starter projects. Click "Use this template" in the top right to create a new repo on GitHub, then follow the instructions in the project's README.
...
```shell
pnpm pkg set "scripts.download-files=tsc && node main.js download-files"
...
Console mode also supports text-only interaction, lets you configure input and output devices, and record sessions to local files. To learn more about console mode, see the [Server startup modes](https://docs.livekit.io/agents/server/startup-modes.md#console-mode) reference.
...
```shell
pnpm pkg set "scripts.dev=tsx main.ts dev"
...
pnpm pkg set "scripts.start=node main.js start"
...
To learn more about `dev` and `start` modes, see the [Server startup modes](https://docs.livekit.io/agents/server/startup-modes.md) reference.

## Deploy to LiveKit Cloud

From the root of your project, run the following command with the LiveKit CLI. Ensure you have [linked your LiveKit Cloud project](#cli) and added the [build and start scripts](#server-startup-modes).
...
This document was rendered at 2026-03-03T03:54:52.032Z.
