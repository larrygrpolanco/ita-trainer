...
const { getActivity } = sharedActivitiesModule as {
  getActivity: (activityId: string) => { maxTurns: number } | undefined;
};
...
function extractTranscriptText(event: unknown): string | null {
...
function isFinalTranscription(event: unknown): boolean {
  const record = asRecord(event);
  if (!record) {
    return true;
  }
...
function evaluateObjective(activityId: string, joinedUserText: string): { objectiveMet: boolean; reason: string } {
...
async function publishObjectiveEvaluation(
  ctx: JobContext,
  message: ObjectiveEvaluationMessage
): Promise<void> {
...
export default defineAgent({
  entry: async (ctx: JobContext) => {
    ...
    session.on(voice.AgentSessionEventTypes.UserInputTranscribed, async (event: unknown) => {
